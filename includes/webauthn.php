...
class WebAuthn {
    ...
    public static function createRegistrationChallenge($userId, $username) {
        ...
        return [
            ...
            'pubKeyCredParams' => [
                ['type' => 'public-key', 'alg' => -7], // ES256 (ECDSA w/ SHA-256) - الأكثر شيوعاً
            ...
            'attestation' => 'none', // 'none' أفضل للموبايل - لا نحتاج attestation
            ...
            'authenticatorSelection' => [
                ...
                'requireResidentKey' => true, // true لحفظ passkeys على الجهاز (مطلوب لأجهزة Android وOPPO و Google Passkeys)
                // لا نحدد authenticatorAttachment - هذا يسمح بجميع أنواع الأجهزة:
                // - 'platform': الأجهزة المدمجة (Face ID, Touch ID, Android biometrics)
                // - 'cross-platform': الأجهزة الخارجية (USB keys, Bluetooth, Google Passkeys)
                // - null/undefined: يسمح بكلاهما (الأفضل للتوافق مع جميع الأجهزة)
            ]
        ];
    }
